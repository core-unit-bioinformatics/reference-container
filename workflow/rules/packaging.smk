...
rule dump_container_readme:
    ...
    run:
        ...
        if readme_txt is None:
            ...
            raise ValueError(
                f"Reference container config does not include a readme section: {wildcards.rc_name_version}"
            )
...
rule create_reference_container_definition_file:
    ...
    run:
        ...
        build_def.write(
            f" {pl.Path(input.manifest).absolute()} /payload/MANIFEST.tsv\n"
        )
        ...
        for fr in file_records:
            build_def.write(
                f" {pl.Path(fr['target_path']).absolute()} {fr['container_path']}\n"
            )
        ...
        if isinstance(input.readme, str) and pl.Path(input.readme).is_file():
            build_def.write(
                f" {pl.Path(input.readme).absolute()} /payload/README.txt\n"
            )
        build_def.write("\n")
        if add_post_section:
...
rule notify_user:
    ...
    run:
        ...
        info_msg += (
            "(Restart the pipeline afterwards to check the container payload)\n\n"
        )
