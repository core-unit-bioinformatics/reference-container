...
def refcon_find_container(manifest_cache, ref_filename):
    ...
    if len(select_container) > 1:
        raise ValueError(
            f'The requested reference file name "{ref_filename}" existis in multiple containers: {select_container}'
        )
    elif len(select_container) == 0:
        raise ValueError(
            f'The requested reference file name "{ref_filename}" existis in none of these containers: {REFCON_NAMES}'
        )
    else:
        pass
...
rule refcon_cache_manifests:
    input:
        manifests=expand(
            "cache/refcon/{refcon_name}.manifest", refcon_name=REFCON_NAMES
        ),
    output:
        cache="cache/refcon/refcon_manifests.cache",
    run:
