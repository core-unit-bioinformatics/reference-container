...
def _get_container_metadata(refcon_name=None):
    ...
    if refcon_name is not None:
        ...
        refcon_md = [
            (key.split("_")[-1], metadata)
            for key, metadata in config.items()
            if key.startswith("metadata_")
        ]
        refcon_md = [
            (name, metadata) for name, metadata in refcon_md if name in select_names
        ]
        if len(refcon_md) > 1:
            raise ValueError(
                f"Ambiguous reference container metadata for name selection: {select_names}"
            )
    else:
        refcon_md = [
            (key.split("_")[-1], metadata)
    ...
    for rc_name, rc_md in refcon_md:
        if "name" not in rc_md or rc_name != rc_md["name"]:
            raise ValueError(
                f"Reference container name missing or mismatch: {rc_name} / {rc_md}"
            )
    return refcon_md
...
def _get_md5_from_file(file_path):
    ...
    assert pl.Path(file_name) == file_path.with_suffix(
        ""
    ), f"Sh*t happened: {file_path} / {file_name}"
    return md5
...
def create_refcon_base_map():
    ...
    for rc_name, rc_md in refcon_md:
        ...
        if rc_version is None:
            raise ValueError(
                f'Key "version" is missing from reference container metadata: {rc_md}'
            )
        try:
            _ = int(rc_version)
        except ValueError:
            raise ValueError(
                f"Reference container version must be a simple integer: {rc_version}"
            )
...
def _find_base_def_file(base_name):
    ...
    if not def_files_folder.is_dir():
        raise ValueError(
            f"Expected to find base def files at path {def_files_folder}, but that does not exist"
        )
    def_file_path = def_files_folder / pl.Path(f"{base_name}.def")
    if not def_file_path.is_file():
        raise ValueError(
            f"Missing def file for base image - expected location: {def_file_path}"
        )
...
def _process_transformation_spec(container_file_name, trans_spec, provider, prefix):
    ...
    if transformation == DataTransformations["decompress"]:
        ...
        source_cache_record = {
            (provider, transformation, source_file_path.name): source_file_path
        }
        transform_local_input = pl.Path(
            f"tmp_{transformation.name}/{provider.name}/{source_file_path.name}"
        )
        transform_pair_record = {
            (provider, transformation, container_file_name): transform_local_input
        }
        ...
        if isinstance(source_info, str):
            ...
            raise ValueError(
                f'Cannot parse "extract" transformation source info: {source_info}'
            )
        ...
        transform_pair_record_member = {
            (provider, transformation, container_file_name, "member"): member_name
        }
        ...
        rule_id = hashlib.md5(
            (source_info["input"] + container_file_name).encode("utf-8")
        ).hexdigest()
        derive_spec = {
        ...
        transform_pair_record = {
            (provider, transformation, container_file_name): derive_spec
        }
    else:
...
def _parse_file_spec(file_spec, check_file_name, provider, prefix):
    ...
    if isinstance(last_record, dict):
        (
            ...
            source_cache_record,
            transform_pair_record,
        ) = _process_transformation_spec(file_spec[0], last_record, provider, prefix)
    ...
    file_identifier = {
        k.name: v for k, v in zip(FileIdentifiers, ["n/a"] * len(FileIdentifiers))
    }
    ...
    for id_name, file_name in zip(FileIdentifiers, file_spec):
        if id_name == FileIdentifiers["name"]:
            ...
            if check_file_name.match(clean_name) is None:
                raise ValueError(
                    f"File name contains illegal characters: {clean_name} --- allowed: {FILE_NAME_CHARS[:-2]}"
                )
            file_identifier[id_name.name] = clean_name
            if source_cache_record is None:
                source_file_path = prefix / pl.Path(file_name)
                source_cache_record = {
                    (provider, DataTransformations["raw"], clean_name): source_file_path
                }
        else:
            if check_file_name.match(file_name) is None:
                raise ValueError(
                    f"File alias contains illegal characters: {id_name.name} / {file_name} --- allowed: {FILE_NAME_CHARS[:-2]}"
                )
...
def _parse_source_listing(source_spec, check_file_names):
    ...
    for file_spec in source_spec["files"]:
        ...
        (
            ...
            source_cache_info,
            transform_pair,
        ) = _parse_file_spec(file_spec, check_file_names, provider, prefix)
        ...
        if any(x in file_names_or_aliases for x in file_identifiers.values()):
            raise ValueError(
                f"Duplicate name or alias in source: {file_identifiers} / {source_spec}"
            )
        ...
        file_record["target_path_md5"] = local_root / pl.Path(
            file_identifiers["name"] + ".md5"
        )
        file_record["container_path"] = container_root / pl.Path(
            file_identifiers["name"]
        )
...
def cache_container_payload():
    ...
    for short_name, rc_md in refcon_md:
        ...
        for source in rc_md["sources"]:
            ...
            ) = _parse_source_listing(source, check_file_names)
            assert all(k not in transform_pair_cache for k in transform_pairs.keys())
            transform_pair_cache.update(transform_pairs)

            local_duplicate_names = local_names_or_aliases.intersection(
                source_file_identifiers
            )
            if local_duplicate_names:
                raise ValueError(
                    f"Duplicate file names or aliases between sources within container: {refcon_name} / {local_duplicate_names}"
                )
            local_names_or_aliases = local_names_or_aliases.union(
                source_file_identifiers
            )

            source_cache.update(source_paths)
            local_payload_cache[refcon_name].extend(
                [fr["target_path"] for fr in source_file_records]
            )
            local_md5_cache[refcon_name].extend(
                [fr["target_path_md5"] for fr in source_file_records]
            )
        ...
        global_duplicate_names = global_names_or_aliases.intersection(
            local_names_or_aliases
        )
...
def build_filename_constraints(which):
    ...
    for provider in DataProviders:
        for transformation in DataTransformations:
            file_names = [
                f
                for p, t, f in filter(filter_keys, name_cache.keys())
                if p == provider and t == transformation
            ]
            if not file_names:
                continue
